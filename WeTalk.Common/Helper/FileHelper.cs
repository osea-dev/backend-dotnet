...
namespace WeTalk.Common.Helper
{
	public class FileHelper : IDisposable
	{
        ...
        *****************************************/
		...
		public static bool CheckFolder(string url)
		{
			url = url + "";
			try
			{
				if (url.ToString().Substring(url.Length - 1, 1) != "\\")
				{
					url = url + "\\";
				}
         ...
         * 调用示列：
         *           string Path = "/url/";    
         *           FileObj.AddFolder(Path);
        *****************************************/
		/// <summary>
		/// 添加新文件夹
		/// </summary>
		/// <param name="url">物理路径</param>
		public static bool AddFolder(string url)
		{
			url = url + "";
			url = url.Replace("\\", "/");
			try
